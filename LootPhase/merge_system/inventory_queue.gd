...
const artifact_base = preload("res://LootPhase/merge_system/ArtifactBase.tscn") 
...
func pickup_item(artifact_type):
	...
	if availableSlot == null: #old code, already checked by calling code
		print("queue is full!")
		return
	artifact.position = Vector2(availableSlot.size.x/2-artifact.texture.get_width()/2, availableSlot.size.y/2-artifact.texture.get_height()/2)
